...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $price = $_POST['price'];
    $boost = 0; // Hidden field, always 0
    $is_approved = 'Pending';
    $user_id = $_SESSION['user_id'];

    // File upload handling for photos and video
    $photo_watch = uploadFile('photo_watch', '../uploads/watches/photos/');
    $photo_certificate = uploadFile('photo_certificate', '../uploads/watches/certificates/');
    $video = uploadFile('video', '../uploads/watches/videos/');

    // Insert data into the database using named placeholders
    $stmt = $conn->prepare("INSERT INTO watches (user_id, title, photo_watch, photo_certificate, video, brand, description, price, boost, is_approved) 
                            VALUES (:user_id, :title, :photo_watch, :photo_certificate, :video, :brand, :description, :price, :boost, :is_approved)");
    if (!$stmt) {
        die('Prepare Error: ' . $conn->errorInfo()[2]);
    }
    $stmt->bindParam(':user_id', $user_id);
    $stmt->bindParam(':title', $title);
    $stmt->bindParam(':photo_watch', $photo_watch);
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Your watch has been successfully posted. Please await admin confirmation.');</script>";
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="text-center">
                            <h2 class="h3 font-weight-normal">Post a Watch</h2>
                        </div>
                        <form method="post" action="post_watches.php" enctype="multipart/form-data">
                            ...
                            <div class="form-group mb-3">
                                <label for="photo_watch">Photo of Watch:</label>
                                <div class="custom-file">
                                    <input type="file" id="photo_watch" name="photo_watch" required>
                                    <label class="custom-file-label" for="photo_watch">Choose file</label>
                                    <span class="file-name" id="photo_watch_name"></span>
                                </div>
                            </div>
                            ...
                            <div class="form-group mb-3">
                                <label for="video">Video of Watch:</label>
                                <div class="custom-file">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="number" min="0" step="1" inputmode="numeric" id="price" name="price" required>
                            </div>

                            <div class="text-center">
                                <button type="submit" class="btn btn-submit">Post Watch</button>
                            </div>
                        </form>
